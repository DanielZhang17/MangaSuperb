...
export default function usePollCharacter(options?: UsePollCharacterOptions) {
  ...
  const previousCharacterRef = useRef<ICharacter | null>(null)
  ...
  const hasCharacterChanged = useCallback((prev: ICharacter | null, current: ICharacter): boolean => {
    if (!prev) return true

    // Compare key fields that indicate a meaningful change
    return (
      prev.image_url !== current.image_url ||
      prev.image_status !== current.image_status ||
      prev.name !== current.name ||
      prev.description !== current.description ||
      prev.optimized_description !== current.optimized_description ||
      prev.image_error !== current.image_error
    )
  }, [])
  ...
  const pollOnce = useCallback(async (id: number) => {
    try {
      ...
      // Check if character has actually changed before calling onUpdate
      const hasChanged = hasCharacterChanged(previousCharacterRef.current, latest)
      if (hasChanged) {
        previousCharacterRef.current = latest
        lastUpdateTimeRef.current = Date.now()
        options?.onUpdate?.(latest)
      }

      // Check for 45-second absolute timeout
      if (startTimeRef.current) {
        const elapsedMs = Date.now() - startTimeRef.current
        if (elapsedMs >= 45000) {
          // 45 seconds elapsed, force update and stop
          if (!hasChanged) {
            options?.onUpdate?.(latest)
          }
          options?.onTimeout?.()
  ...
  }, [intervalMs, maxAttempts, options, stopPolling, terminalStatusesOpt, hasCharacterChanged])
