...
import { AutoRunBanner } from './components/auto-run-banner'
...
export default function ComicsPage() {
  ...
  const autoRunState = useAutoRun()
  const activeGeneratingAutoRun = autoRunState.autoRun?.status === 'queued' || autoRunState.autoRun?.status === 'running'
    ...
    <div className="flex-1">
      <Tabs value={workflowMode} onValueChange={(value) => setWorkflowMode(value as 'auto' | 'pro')}>
        ...
        <TabsContent value="pro" className="mt-0">
          {activeGeneratingAutoRun && autoRunState.autoRun && (
            <AutoRunBanner
              autoRun={autoRunState.autoRun}
              onReturn={() => setWorkflowMode('auto')}
            />
          )}
