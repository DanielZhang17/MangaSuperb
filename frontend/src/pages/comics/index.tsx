...
import { useComicDetail } from '@/hooks/use-comics'
...
export default function ComicsPage() {
  ...
  const [currentComicId] = useAtom(currentComicIdAtom)
  ...
  const { comic: fetchedComic } = useComicDetail(currentComicId ?? undefined)
  ...
  useEffect(() => {
    if (!fetchedComic?.id) return
    if (comicDetail?.id === fetchedComic.id && comicDetail?.updated_at === fetchedComic.updated_at) return
    setComicDetail(fetchedComic)
  }, [comicDetail?.id, comicDetail?.updated_at, fetchedComic, setComicDetail])
