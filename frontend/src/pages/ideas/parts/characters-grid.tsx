  ...
  return (
    <div className="flex w-full flex-col gap-4">
      ...
      {!loading && !error && characters.length === 0 && (
        <div className="rounded-xl border border-dashed p-6 sm:p-10 text-center text-muted-foreground text-sm">
          暂无人 物，去创建一个吧！
        </div>
      )}
  ...
  return (
    <Card className="overflow-hidden relative">
      <button
        type="button"
        aria-label="删除"
        className="absolute right-2 top-2 z-10 inline-flex h-9 w-9 sm:h-8 sm:w-8 items-center justify-center rounded-full bg-background/70 hover:bg-background text-muted-foreground hover:text-foreground shadow transition-colors"
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="max-w-xs">
          <div className="py-4">
            ...
            <Button variant="destructive" className="w-full min-h-[44px]" onClick={handleDelete}>
              确定
            </Button>
          </div>
        </DialogContent>
