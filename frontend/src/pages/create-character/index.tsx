...
export default function CharacterCreatorPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full min-h-screen p-4 sm:p-6 md:p-8 bg-background text-foreground gap-6 lg:gap-0">
      <div className="flex flex-col w-full lg:w-1/2 space-y-4 sm:space-y-6 lg:pr-8">
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <h1 className="text-xl sm:text-2xl font-semibold">{String(t('createCharacter:title.new'))}</h1>
          <div className="flex items-center space-x-2">
            <Label htmlFor="ai-optimize" className="text-sm text-muted-foreground">{String(t('createCharacter:aiOptimize'))}</Label>
            <Switch id="ai-optimize" checked={optimize} onCheckedChange={setOptimize} />
          </div>
        </header>
        <div>
          <Button variant="secondary" className="w-full sm:w-auto">{String(t('createCharacter:random'))}</Button>
        </div>
        <Textarea
        ...
        <div className="space-y-3">
          <div className="flex items-baseline justify-between">
            <h2 className="text-base sm:text-lg font-medium">{String(t('createCharacter:reference.title'))}</h2>
            <span className="text-xs sm:text-sm text-muted-foreground">{String(t('createCharacter:reference.count', { count: files?.length ?? 0 }))}</span>
          </div>
