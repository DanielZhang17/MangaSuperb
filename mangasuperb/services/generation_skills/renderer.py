...
class PromptRenderer:
    def render_page_prompt(self, resolved: ResolvedGenerationContext) -> str:
        ...
        sections = [
            ...
            self._section(
                ...
                [
                    f"Visual mode: {self._visual_mode_label(constraints.visual_mode)}",
                    f"Source: {constraints.visual_mode_source}",
                ],
            ),
        ]
        if constraints.character_locks:
            sections.append(
                self._section(
                    "Character locks / Character Locks",
        ...
        sections.extend(
            ...
            self._section(
                ...
                ["Panel-by-Panel Content", *self._panel_lines(resolved)],
            ),
    ...
    def _visual_mode_label(self, visual_mode: str | None) -> str:
        if visual_mode == "black-white":
            return "black-white manga linework"
        if visual_mode == "color":
            return "color comic rendering"
        return str(visual_mode or "unspecified")
