...
def _debug_logging_enabled() -> bool:
    """Return True if prompt logging is enabled via DEBUG env or Flask config."""
    env_flag = os.getenv("DEBUG")
    if env_flag is not None:
        return env_flag.strip().lower() == "true"

    try:
        return bool(current_app.config.get("DEBUG", False))  # type: ignore[attr-defined]
    except RuntimeError:
...
def _summarize_content_part(part: Any, idx: int) -> str:
    ...
    if isinstance(part, str):
        return f"[text {idx}] {part}"

    if isinstance(part, dict):
        if "text" in part and isinstance(part["text"], str):
            return f"[text {idx}] {part['text']}"
    ...
    return f"[part {idx}] {repr(part)[:200]}"


def log_gemini_contents(contents: Iterable[Any], model_name: str, context: str = "") -> None:
    """Write prompt contents to logs/gemini_prompts.log when DEBUG is enabled."""
