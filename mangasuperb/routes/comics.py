...
def get_comic_images(comic_id: int) -> Any:
    ...
    # Allow owner access when authed; otherwise only allow public comics
    is_owner = current_user.is_authenticated and comic.user_id == current_user.id
    if not is_owner and not comic.is_public:
        return jsonify({"error": "Comic not found"}), 404
...
def publish_comic(comic_id: int) -> Any:
    ...
    render_stage = (
        ComicWorkflowStage.query.filter_by(comic_id=comic_id, stage="render")
        .order_by(ComicWorkflowStage.id)
        .first()
    )
    if render_stage:
        db.session.refresh(render_stage)
    ...
    if (
        (not render_stage or render_stage.status != "completed")
        and previously_published_at
    ...
    workflow_already_published = (
        comic.workflow_status == "completed"
        and render_stage is not None
        and render_stage.status == "completed"
    )
    render_not_newer_than_publish = (
        bool(last_render_ts)
        and bool(previously_published_at)
        and last_render_ts <= previously_published_at
    )

    if (
        previously_published_at
        and (workflow_already_published or render_not_newer_than_publish)
        and not assets_missing
        and not visibility_changing
    ):
        current_app.logger.info(
    ...
    except Exception:  # pragma: no cover - queue failure
